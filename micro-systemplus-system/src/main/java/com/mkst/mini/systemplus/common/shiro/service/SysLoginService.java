package com.mkst.mini.systemplus.common.shiro.service;
...
import com.mkst.mini.systemplus.common.enums.UserStatus;
...
import com.mkst.mini.systemplus.framework.util.MessageUtils;
import com.mkst.mini.systemplus.framework.util.ServletUtils;
import com.mkst.mini.systemplus.framework.web.exception.user.*;
...
public class SysLoginService
{
     ...
     * 登录
     */
    public SysUser loginNoPassword(String username, LoginType loginType){
        // 用户名或类型为空 错误
        if (StringUtils.isEmpty(username)||StringUtils.isEmpty(loginType))
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(username, Constants.LOGIN_FAIL, MessageUtils.message("not.null")));
            throw new UserNotExistsException();
        }
        SysUser user = null;
        if(loginType.getCode().equals(LoginType.CAS.getCode())||loginType.getCode().equals(LoginType.YZJ.getCode())||loginType.getCode().equals(LoginType.OAUTH2.getCode())){
            user = userService.selectUserByLoginName(username);
        }else {
            // 根据第三方系统账号查询当前系统用户信息
            SysUser query = new SysUser();
            query.setLoginName(username);
            query.setUserType(loginType.getCode());
            user = userService.selectUserByLoginNameAndType(query);
        }
